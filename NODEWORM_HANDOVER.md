...
- **needs-key (software done):** `captureTraffic`, managed session, hosted connector, co-browse all need `BROWSERBASE_API_KEY` (+ Signal bridge) in Vercel prod to run live. `EXECUTE_SIGNING_KEY` needed for build/tunnel plans. LLM discovery needs an OpenRouter/free key (else heuristics).
