...
function buildPrompt(t: PortalTask): string {
  ...
  return [
    ...
    ``,
    `Do exactly this:`,
    `1. First check the start page loaded a real developer portal. If it shows a "page not found" / 404, an error, or a raw API/JSON response instead of a normal web page, DO NOT stay there: open a search engine, search "${t.appName} developer portal create OAuth app register application", and navigate to ${t.appName}'s official developer portal or API-credentials page before continuing.`,
    `2. If you land on a sign-in, login, password, 2FA, or CAPTCHA page, DO NOT type anything and DO NOT attempt to log in. A human is watching this same browser and will sign in. Wait, then re-check the page every few steps until you are past the login wall, then continue.`,
    `4. Find where to create a new OAuth app / API client / integration (often "Developer settings", "Apps", "OAuth apps", "API credentials", or "Create app").`,
    `5. Create a new app named "NodeWorm" (or "${t.appName} via NodeWorm" if a name is required and "NodeWorm" is taken).`,
    `6. Set the redirect URI / callback URL / authorized redirect to EXACTLY this, copying it character-for-character: ${t.redirectUri}`,
    `7. ${scopeLine}`,
    `8. Save / create the app.`,
    `9. Open the app's settings and read its Client ID and Client Secret (may be called App ID / API Key and Secret / Consumer Key and Secret). If the secret is only shown once, reveal and read it now.`,
    ``,
